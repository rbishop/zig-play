...
pub fn main() u8 {
    ...
    var parse_ret = c.phr_parse_request(req, req_len,
                                        @ptrCast(?[*]?[*]u8, &method),
                                        @ptrCast(?[*]usize, &method_len),
                                        @ptrCast(?[*]?[*]u8, &path),
                                        @ptrCast(?[*]usize, &path_len),
                                        @ptrCast(?[*]c_int, &minor_version),
                                        @ptrCast(?[*]c.phr_header, &headers),
                                        @ptrCast(?[*]usize, &num_headers),
                                        0);
